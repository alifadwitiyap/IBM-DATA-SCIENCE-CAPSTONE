...
#helper function
def populate_dropdown_options(spacex_df):
    dropdown_option=[{'label': 'All Sites', 'value': 'ALL'}]

    for site in spacex_df['Launch Site'].unique():
     dropdown_option.append({'label': site, 'value': site})

    return dropdown_option


dropdown_option=populate_dropdown_options(spacex_df)
...
def update_scatter_chart(selected_site, payload_range):
    if selected_site == 'ALL':
        ...
        filtered_df = spacex_df[(spacex_df['Launch Site'] == selected_site) & spacex_df['Payload Mass (kg)'].between(payload_range[0], payload_range[1])]
